...
$stmt = $db->prepare(
    "SELECT d.*,
     ...
     FROM favorites f JOIN dishes d ON d.dish_id = f.dish_id
...
<section class="section" style="padding-top: 10px;">
    <div class="container">
        <div class="section-head"><h2><?= t('favorites_title') ?></h2></div>
        <?php if (!$dishes): ?>
            <div class="empty-state"><div class="icon">🤍</div><p><?= t('no_favorites') ?></p></div>
        <?php else: ?>
        <div class="dish-grid">
            <?php foreach ($dishes as $dish): ?>
                <div class="card dish-card">
                    <div class="dish-media">
                        <button class="wishlist-toggle active" data-favorite data-dish-id="<?= $dish['dish_id'] ?>">❤️</button>
                        <?= dishImageHtml($dish) ?>
                    </div>
                    <div class="dish-body">
                        <h3><?= esc($dish['name']) ?></h3>
